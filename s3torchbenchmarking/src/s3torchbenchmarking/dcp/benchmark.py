...
def run_benchmark(cfg: DictConfig) -> dict:
    ...
    metrics = {
        "throughput_mibs": throughput_mibs.describe().dropna().to_dict(),
        "corrected_save_durations_s": corrected_save_durations_s.describe()
        .dropna()
        .to_dict(),
        "processing_durations_s": processing_durations_s.describe().dropna().to_dict(),
    }
