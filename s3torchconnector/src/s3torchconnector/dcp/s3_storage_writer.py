...
class S3StorageWriter(StorageWriter):
    ...
    def write_data(
        ...
        def gen_object_key():
            nonlocal obj_count
            object_key = f"{storage_plan.prefix}{obj_count}{DEFAULT_SUFFIX}"
            obj_count += 1
            return object_key
        ...
        for rank in range(self.thread_count):
            t = threading.Thread(
                    ...
                    args = (objects_queue, results_queue, rank)
                    )
            t.start()
            threads.append(t)
    ...
    def finish(self, metadata: Metadata, results: List[List[WriteResult]]):
        if self.is_coordinator:
            ...
            for wr_list in results:
                s3_storage_metadata.update({wr.index: wr.storage_data for wr in wr_list})
