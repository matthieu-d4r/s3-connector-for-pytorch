...
def test_init():
    s3fs = S3FS(TEST_REGION)

    assert s3fs.path is None
    assert s3fs.region == TEST_REGION
    assert s3fs.client is not None
    assert s3fs.checkpoint is not None


@pytest.mark.skip(reason="not testable as-is (requires contract changes in `S3Checkpoint`)")
...
def test_create_stream_raise_when_mode_is_unknown():
    ...
    with pytest.raises(ValueError) as excinfo:
        with s3fs.create_stream("path", "foo"):
            pass

    assert str(excinfo.value) == "Invalid mode argument, create_stream only supports rb (read mode) & wb (write mode)"
...
def test_exists_true_when_key_exists():
    test_key = "test_exists"
    path = f"s3://{TEST_BUCKET}/{test_key}"

    mock_client = MockS3Client(TEST_REGION, TEST_BUCKET)
    mock_client.add_object(test_key, b"data")
    s3fs = S3FS(TEST_REGION, mock_client)

    assert s3fs.exists(path) is True


def test_exists_false_when_key_does_not_exist():
    test_key = "test_does_not_exist"
    path = f"s3://{TEST_BUCKET}/{test_key}"
